...
def shorten_emojify(string):
    ...
    rules_priority = {
        "(?:, )?dazu eine Salatschale": "🥗",
        "(?:, )?dazu drei beilagen": "+3x🍛",
        "(?:, )?dazu Salat und Dessert": "🥗🍮",
    }
    rules = {
        "salat": "🥗",
        "dessert": "🍮",
...
class Gericht:
    ...
    def __str__(self):
        return (
            self.beautiful_description()
            + "\t"
            + self.price
            + "\t"
            + str(self.tags_as_emoji())
        )

    def beautiful_description(self):
        removed_tags = re.sub(
            "\([0-9abcdefg,]*\)", "", self.description
        )  # removes tags like "(1a)","(15,18,20b)"
    ...
    def short_description(self):
        ...
        for rule in strip:
            desc = re.sub(
                comma_strip + rule + comma_strip, "", desc, flags=re.IGNORECASE
            )
